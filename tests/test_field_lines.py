...
from pycocos.coordinates.compute_coordinates import _compute_surface_coordinate_row
...
def test_circular_surface_rows_satisfy_boozer_identities():
    ...
    # Theta is obtained by cumulative quadrature, so the identities involving
    # grad(theta) retain the corresponding finite-resolution error.
    np.testing.assert_allclose(lhs[0], rhs[0], rtol=2.0e-3, atol=1.0e-6)
    np.testing.assert_allclose(lhs[3], rhs[3], rtol=2.0e-3, atol=1.0e-6)
    for index in (1, 2, 4):
